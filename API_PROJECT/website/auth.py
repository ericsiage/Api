...
def login():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first() #filtrar todos los usuarios que tienen eso email=email.
        if user:
            if check_password_hash(user.password, password):
                flash('Logged in successfully!', category='success')
                login_user(user, remember=True) #Remember si user estaba conectado.
